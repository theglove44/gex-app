...
def apply_base_theme() -> None:
    ...
    <style>
        :root {{
            ...
            --bg-secondary: {config.COLORS['bg_secondary']};
            --bg-card: {config.COLORS['bg_card']};
            --bg-elevated: {config.COLORS['bg_elevated']};
            ...
            --lime: {config.COLORS['lime']};
            --orange: {config.COLORS['orange']};
            --purple: {config.COLORS['purple']};
            --yellow: {config.COLORS['yellow']};
            --call-gex: {config.COLORS['call_gex']};
            --put-gex: {config.COLORS['put_gex']};
            --spot-line: {config.COLORS['spot_line']};
            --zero-gamma: {config.COLORS['zero_gamma']};
            --gradient-card: {config.COLORS['gradient_card']};
        }}
...
def get_dark_layout() -> dict:
    ...
    return dict(
        ...
        xaxis=dict(
            ...
            zerolinecolor=config.COLORS['bg_elevated'],
            tickfont=dict(color=config.COLORS['text_secondary'], size=11),
            title_font=dict(color=config.COLORS['text_secondary'], size=12)
        ),
        yaxis=dict(
            gridcolor='rgba(48,54,61,0.5)',
            zerolinecolor=config.COLORS['text_muted'],
            zerolinewidth=2,
            tickfont=dict(color=config.COLORS['text_secondary'], size=11),
            title_font=dict(color=config.COLORS['text_secondary'], size=12)
        ),
        hoverlabel=dict(
            ...
            bordercolor=config.COLORS['bg_elevated'],
            font=dict(color=config.COLORS['text_primary'], size=13)
        ),
...
def add_common_vertical_markers(fig: go.Figure, result: GEXResult) -> go.Figure:
    ...
    if result.call_wall:
        ...
        fig.add_annotation(
            ...
            showarrow=False,
            font=dict(color=config.COLORS['call_gex'], size=9),
    ...
    if result.put_wall:
        ...
        fig.add_annotation(
            ...
            showarrow=False,
            font=dict(color=config.COLORS['put_gex'], size=9),
    ...
    if pos_mask.any():
        fig.add_trace(go.Bar(
            ...
            marker=dict(
                ...
                line=dict(color=config.COLORS['call_gex'], width=1)
            ),
            name='Call GEX',
            hovertemplate=(
    ...
    if neg_mask.any():
        fig.add_trace(go.Bar(
            ...
            marker=dict(
                ...
                line=dict(color=config.COLORS['put_gex'], width=1)
            ),
            name='Put GEX',
            hovertemplate=(
...
def render_metric_card(label: str, value: str, card_type: str = "neutral", subtitle: str = "") -> str:
    ...
    subtitle_html = f'<p style="color:{config.COLORS["text_muted"]};font-size:0.7rem;margin:0.25rem 0 0 0;">{subtitle}</p>' if subtitle else ""
